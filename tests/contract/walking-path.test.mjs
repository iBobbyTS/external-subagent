...
test('CLI requires a known agent and rejects dsh before prompt dispatch', async () => { const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'external-subagent-agent-contract-')); try { await assert.rejects(() => runCli('/missing/socket', 'spawn', { repository: directory, prompt: 'hello' }), /agent is required/); await assert.rejects(() => runCli('/missing/socket', 'spawn', { agent: 'dsh', repository: directory, prompt: 'hello' }), /prompt_count=0/); await assert.rejects(() => runCli('/missing/socket', 'spawn', { agent: 'zcode', repository: directory, prompt: 'hello', extra: true }), /unsupported field/); } finally { await fs.rm(directory, { recursive: true, force: true }); } });
