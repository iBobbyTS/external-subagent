...
test('CLI walking path uses RPC spawn wait result close', async () => { const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'external-subagent-walking-')); const socket = path.join(directory, 'daemon.sock'); const server = await fixtureServer(socket); try { const created = await runCli(socket, 'spawn', { agent: 'zcode', repository: directory, prompt: 'hello' }); assert.deepEqual(created.result, { agent_id: 10000001, submission_disposition: 'admitted', phase: 'running' }); const waited = await runCli(socket, 'wait', { agent_id: 10000001, wait_time: 0 }); assert.equal(waited.result.result.final_text, 'fixture result'); const result = await runCli(socket, 'result', { agent_id: 10000001 }); assert.equal(result.result.result.complete, true); const closed = await runCli(socket, 'close', { agent_id: 10000001 }); assert.equal(closed.result.task.closed, true); assert.equal(server.promptCount(), 1); } finally { await new Promise((resolve) => server.close(resolve)); await fs.rm(directory, { recursive: true, force: true }); } });

test('CLI rejects malformed spawn fields before transport', async () => { const directory = await fs.mkdtemp(path.join(os.tmpdir(), 'external-subagent-agent-contract-')); try { await assert.rejects(() => runCli('/missing/socket', 'spawn', { agent: null, repository: directory, prompt: 'hello' }), /agent must be omitted/); await assert.rejects(() => runCli('/missing/socket', 'spawn', { agent: 'zcode', model: null, repository: directory, prompt: 'hello' }), /model must be omitted/); await assert.rejects(() => runCli('/missing/socket', 'spawn', { agent: 'zcode', repository: directory, prompt: 'hello', extra: true }), /unsupported field/); } finally { await fs.rm(directory, { recursive: true, force: true }); } });
