...
test('updateInstallation switches between two independent candidate roots', () => {
  ...
  assert.match(state.active.entry_sha256, /^[0-9a-f]{64}$/u);
  fs.rmSync(a,{recursive:true,force:true}); fs.rmSync(b,{recursive:true,force:true}); fs.rmSync(data,{recursive:true,force:true});
});

test('update rejects non-executable or escaping candidate entry while preserving active', () => {
  const data = fs.mkdtempSync(path.join(os.tmpdir(), 'update-entry-')); const p={data,state:path.join(data,'state.json')};
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'candidate-entry-')); fs.mkdirSync(path.join(root,'npm/native/darwin-arm64'),{recursive:true}); fs.mkdirSync(path.join(root,'bin'));
  fs.writeFileSync(path.join(root,'package.json'), JSON.stringify({version:'1.0.0'}));
  const files=[]; for (const name of ['external-subagentd','external-subagent-mcp']) { const b=Buffer.alloc(32); b.writeUInt32LE(0xfeedfacf); b.writeUInt32LE(0x0100000c,4); fs.writeFileSync(path.join(root,'npm/native/darwin-arm64',name),b,{mode:0o755}); files.push({name,bytes:b.length,sha256:crypto.createHash('sha256').update(b).digest('hex')}); }
  fs.writeFileSync(path.join(root,'npm/native/darwin-arm64/payload.json'),JSON.stringify({schema_version:1,product:'external-subagent',platform:'darwin-arm64',version:'1.0.0',files}));
  fs.writeFileSync(path.join(root,'bin/external-subagent.mjs'),'#!/usr/bin/env node\n',{mode:0o644});
  assert.throws(() => updateInstallation(p,{version:'1.0.0',candidateRoot:root,platform:'darwin-arm64',availableVersions:['1.0.0']}), /candidate stable entry must be executable/);
  assert.equal(JSON.parse(fs.readFileSync(p.state,'utf8')).active, null);
  fs.rmSync(root,{recursive:true,force:true}); fs.rmSync(data,{recursive:true,force:true});
});
