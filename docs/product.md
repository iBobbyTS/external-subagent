...
plugin manifest 的 `version` 是 Codex 全局 content-store 的缓存身份（`plugin@marketplace@version`），每个发布候选必须携带独立版本（当前 `0.1.1`），否则已缓存同一身份的其他 home 会覆盖新候选的字节；详见 [compatibility/codex.md](compatibility/codex.md)。
